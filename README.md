...
- 일반 화면 오버레이와 잠금화면 보호를 각각 켜고 끄는 독립 설정
- 일반 앱 사용 중 및 잠금화면에서 전체 화면 터치 차단
- 오버레이 불투명도 0~100% 조절 및 전체 화면 미리보기
- 필요한 권한이 없을 때만 표시되는 설정 안내
- 사용하지 않는 앱 관리 활성화 감지 및 설정 바로가기(Android 11 이상)
- 상시 알림 설정 바로가기
...
2. 일반 앱 화면을 보호하려면 **일반 화면 오버레이**를 켭니다. 권한이 없으면 나타나는 **다른 앱 위에 표시 권한 설정** 버튼으로 권한을 허용합니다.
3. 잠금화면을 보호하려면 **잠금화면 보호**를 켭니다. 권한이 없으면 나타나는 **잠금화면 접근성 서비스 설정** 버튼으로 `Reverse Portrait Guard`를 활성화합니다.
4. **오버레이 및 알림** 카드에서 불투명도를 조절하거나 미리보기 및 상시 알림 설정을 엽니다.
5. **사용하지 않는 앱 관리** 경고가 표시되면 설정 버튼을 눌러 해당 기능을 끕니다.

외부 APK의 접근성 설정이 제한된 경우 앱 정보 화면의 우측 상단 메뉴에서 **제한된 설정 허용**을 먼저 선택해야 할 수 있습니다.

설정 화면은 자동으로 열리지 않습니다. 앱에 표시되는 각 설정 버튼을 직접 눌러 이동해야 합니다.
...
- Android 10 이하에서는 사용하지 않는 앱 관리 상태를 앱에서 직접 확인하지 않습니다.
